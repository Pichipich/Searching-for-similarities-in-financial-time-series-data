...
for i in range(num_symbols):
    for j in range(i + 1, num_symbols):
        tau, p_value = kendalltau(returns_data[i], returns_data[j])
        if np.isnan(tau):
            tau = 0
        distance = 0.5 * (1 - tau)
...
# Example output of the distance matrix
...
def compute_distance_matrix(data):
    ...
    for i in range(num_samples):
        for j in range(i + 1, num_samples):
            # Calculate Kendall's Tau between each pair of sequences
            tau, p_value = kendalltau(data[i], data[j])
            # Use 1 - tau to convert correlation to distance
            distance_matrix[i, j] = distance_matrix[j, i] = 1 - tau
...
num_clusters = 22  # Adjust the number of clusters based on your specific analysis
...
ax.set_title('Cluster Composition by Company - [K-Medoids - Kendall\'s Tau]', fontsize=20)
