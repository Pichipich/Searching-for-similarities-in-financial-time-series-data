...
# Assuming `equal_length_data` contains time series data of equal length
dtw_distance_matrix = np.zeros((len(symbols), len(symbols)))
...
# Ensure diagonal is zero after computations
np.fill_diagonal(dtw_distance_matrix, 0)

linkage_matrix = linkage(squareform(dtw_distance_matrix), method='average')
...
for k in range(2, 20):
    ...
    silhouette_scores_original.append(silhouette_score(dtw_distance_matrix, clusters, metric='precomputed'))
...
threshold = 0.8 * max_merge_distance
...
plt.title('Ηierarchical Clustering with DTW Distance')
plt.show()
...
plt.title('MDS Clustering Results [Hierarchical With DTW]', fontsize=16)
plt.grid(True)
...
def temporal_cluster_validation(fund_data, window_size, step_size, num_clusters):
    ...
    return np.array(cluster_labels_over_time).T  # Transpose to make rows correspond to funds
...
plt.suptitle('Cluster Evolution Over Time [Hierarchical - DTW]', fontsize=35, va='top', ha='center', x=0.5, y=0.95)
...
ax.set_title('Cluster Composition by Company - [Hierarchical - DTW]', fontsize=22)
...
# Check if there's only one subplot (one cluster)
